                ...
                vertexOutput vert(vertexInput v) {
                    ...
                    o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                        ...
                        o.surfaceColor = float4(mainTexCol * _Color * (diffuseCol + specularCol), 1);
                            ...
                            o.surfaceColor.rgb *= IBLRefl(_Cube, _Detail, worldRefl, _ReflectionExposure, _ReflectionFactor); //here we respect the lighting model so we multiply the result
                        #endif
                    #else
                        #if _IBLMODE_REFL
                            float3 worldSpaceViewDir = normalize(_WorldSpaceCameraPos - o.posWorld);
                            float3 worldRefl = reflect(-worldSpaceViewDir, o.normalWorld.xyz);
                            o.surfaceColor.rgb += IBLRefl(_Cube, _Detail, worldRefl, _ReflectionExposure, _ReflectionFactor); //here we don't have a lighting model so we add the result
                        #endif
                    #endif
                    return o;
                }
                ...
                half4 frag(vertexOutput i) : COLOR {
                    float4 finalColor = float4(0,0,0,_Color.a);
                        ...
                        finalColor.rgb += mainTexCol * _Color * diffuseCol + specularCol;
                            ...
                            finalColor.rgb *= IBLRefl(_Cube, _Detail, worldRefl, _ReflectionExposure, _ReflectionFactor); //here we respect the lighting model so we multiply the result
                        #endif
                    #elif _LIGHTING_VERT
                        finalColor = i.surfaceColor;
                            ...
                            float3 worldSpaceViewDir = normalize(_WorldSpaceCameraPos - i.posWorld);
                            float3 worldRefl = reflect(-worldSpaceViewDir, worldNormalAtPixel);
                            finalColor.rgb += IBLRefl(_Cube, _Detail, worldRefl, _ReflectionExposure, _ReflectionFactor); //here we don't have a lighting model so we add the result
                        #endif
                    #endif
